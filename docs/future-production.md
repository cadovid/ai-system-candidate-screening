...
As of 2026-09-02, the current Commission timeline describes broad AI Act application and transparency/enforcement activity from 2 August 2026, while Annex III high-risk employment rules are described as applying from 2 December 2027. Those dates are an orientation point, not a classification of this system. A concrete recruitment deployment must be reviewed against the current text and implementation material.
...
- Introduce a queue and worker for provider interpretation, retries, summary generation, and outbound reminders. Preserve the current invariant: no database transaction is held open while waiting on a model. Add timeouts, exponential backoff, circuit breaking, dead-letter handling, and provider failover policy.
...
- Treat every hosted route as an explicit, reviewed deployment choice. The current recommended free development route is Groq `openai/gpt-oss-20b`; verify its organization/project RPM, RPD, TPM, and TPD limits, model lifecycle, structured-output behavior, retention, residency, and processor terms at each release. The secondary OpenRouter route is `openrouter/free`, which dynamically selects an available free model; review its current model catalog, structured-output compatibility, quota, downstream retention, residency, and processor terms at each release. OpenAI remains an explicit usage-billed option. Do not add silent fallback from a free model to a paid model; define any production failover policy with separate cost, privacy, and human-approval controls.
...
- Export structured logs, traces, and metrics with correlation IDs. Redact content at the logger boundary. Alert on provider failures, queue age, latency, database errors, optimistic conflicts, guardrail spikes, summary fallback rate, reminder complaints, and review backlog.
