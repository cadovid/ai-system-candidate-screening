...
- Treat the OpenRouter `deepseek/deepseek-v4-flash:free` route as development-only: verify its active endpoint, structured-output support, quota, downstream retention, residency, and processor terms at each release. Do not add silent fallback from a free model to a paid model; define an explicit, approved failover policy with separate cost and privacy controls.
