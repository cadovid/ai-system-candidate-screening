...
async def test_provider_failure_is_replayable_and_preserves_canonical_state(
    ...
    coordinator.interpreter = _NameInterpreter(fail=True)
    created = await coordinator.create_conversation(language=Language.EN)

    failed = await coordinator.process_turn(created.conversation_id, "Ada", "retry-key")
    replayed = await coordinator.process_turn(created.conversation_id, "Ada", "retry-key")
